function [sol,Z]=MAR_Shaffer(nt,dur,Prec,nprec,nsuc,nrec,rec,recu)
tic
...
%                    end
    ...
    if tp~=0
        menorEF=0;
        for j=1:nt
            if ES(j)+1<tp && EF(j)>tp
                if menorEF==0 || EF(menorEF)>EF(j)
                    for k=1:nrec
        ...
        for j=1:nt
            if ES(j)+1<tp && EF(j)>tp
                if mayorLS==0 || LS(mayorLS)<LS(j)
                    if j~=menorEF && Prec(menorEF,j)==0
                        if rec(k,j)>=constrec(k)
                            mayorLS=j;
                            break;
                        end
